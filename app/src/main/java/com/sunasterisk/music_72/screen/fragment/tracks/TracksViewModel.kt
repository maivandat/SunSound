...
class TracksViewModel(private val repository: TrackRepositoryImplementor) : ViewModel() {
    ...
    fun getTracks(genre: String, limitItem: Int) {
        if (genre == GenreName.ALL_TRACK) getAllTrack(limitItem)
        else getTracksByGenre(genre, limitItem)
    }

    private fun getTracksByGenre(genre: String, limitItem: Int) {
        repository.getTracksByGenre(genre.toLowerCase(Locale.getDefault()), LIMIT_ITEM + limitItem)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                _tracks.value = it
    ...
    private fun getAllTrack(limitItem: Int) {
        repository.getAllTrack(LIMIT_ITEM + limitItem)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                _tracks.value = it
